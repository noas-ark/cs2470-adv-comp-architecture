"""Insert CS2470Profile_* record_function annotations into HF Gemma 2 (transformers 4.57).

Same approach and label names as annotate_llama.py so parse_profile.py works unchanged.
Always re-patches from a pristine backup, so it is safe to run repeatedly.
Usage: python annotate_gemma2.py            # apply annotations
       python annotate_gemma2.py --restore  # restore original file
"""
import os
import re
import shutil
...
core = re.search(r"        attn_output, attn_weights = attention_interface\(\n.*?\n        \)", src, re.S).group(0)
wrap(core, "AttentionInterface")  # Gemma 2 may run eager attention, so this is not labeled SDPA
wrap("        attn_output = self.o_proj(attn_output)", "OProj")

# 2) Model-level annotations
wrap("            inputs_embeds = self.embed_tokens(input_ids)", "Embedding")
wrap("        logits = self.lm_head(hidden_states[:, slice_indices, :])", "LMHead")

# 3) Whole-module annotations (wrap each forward body)
for cls, label in [
    ("Gemma2RMSNorm", "RMSNorm"),
    ("Gemma2RotaryEmbedding", "RotaryEmbedding"),
    ("Gemma2MLP", "MLP"),
    ("Gemma2Attention", "Gemma2Attention"),
    ("Gemma2DecoderLayer", "DecoderLayer"),
]:
